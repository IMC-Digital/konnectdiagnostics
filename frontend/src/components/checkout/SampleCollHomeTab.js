...
export default function SampleCollHomeTab({
  ...
  useEffect(() => {
    const fetchUserAddresses = async () => {
      try {
        const response = await axios.get(
          `${BASE_API_URL}/user/get-user-addresses`,
    ...
    const getAltMob = async () => {
      try {
        const response = await axios.get(`${BASE_API_URL}/user/get-altmobi`, {
          params: { userid: userId },
        });
  ...
  const selectAddress = (e) => {
    ...
    const selectedAddressObject = userAddresses.filter(
